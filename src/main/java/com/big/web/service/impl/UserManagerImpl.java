package com.big.web.service.impl;
...
import com.big.web.service.UserService;
 ...
 */
@Service("userManager")
@WebService(serviceName = "UserService", endpointInterface = "com.big.web.service.UserService")
public class UserManagerImpl extends GenericManagerImpl<User, Long> implements UserManager, UserService {
     ...
     * @see com.big.web.service.MailEngine#sendMessage(org.springframework.mail.SimpleMailMessage, String, java.util.Map)
     */
...
//        sendUserEmail(user, passwordRecoveryTemplate, url);
        ...
        try {
        	...
        	// TODO avoid multiple click to reset
        	boolean valid = true;
        	
        	//check EMAIL table, if exist within 15 menit throw error
        	
        	if (valid)
        		emailManager.sendUserEmail(true, "Password Recovery", user, passwordRecoveryTemplate, url);
     ...
     */
    @Override
    public User updatePassword(final String username, final String currentPassword, final String recoveryToken, final String newPassword, final String applicationUrl, final boolean sendEmail) throws UserExistsException {
        ...
        if (isRecoveryTokenValid(user, recoveryToken)) {
            ...
            if (sendEmail)
            try {
				emailManager.sendUserEmail(true, "Password Updated", user, passwordUpdatedTemplate, applicationUrl);
			} catch (Exception e) {
				e.printStackTrace();
			}
