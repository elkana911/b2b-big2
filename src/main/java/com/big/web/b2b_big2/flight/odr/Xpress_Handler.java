...
public class Xpress_Handler extends BasicAirlineHandler implements IAirlineHandler {

	private static Xpress_Handler self = null;

	public static Xpress_Handler getInstance(ISetupDao setupDao){
		if (self == null)
			self = new Xpress_Handler(setupDao);
		
		return self;
	}
	
	private Xpress_Handler(ISetupDao setupDao) {
		super(Airline.EXPRESS, setupDao);
	}
	...
	private int convertToInt(BigDecimal value){
		return Utils.convertToInt(value, Airline.EXPRESS);
	}
...
//		sumBasicFare += Utils.convertToInt(fareInfo2.getChildFare(), airline) * childCount;
		...
		// express hasil scraping tidak ada data tax, jd tinggal dihitung dari total fare
		if (adultCount > 0){
			double dAdultTax = convertToInt(fareInfo2.getRate()) * 0.1;
			BigDecimal adultTax = new BigDecimal(dAdultTax);
			adultTax = adultTax.setScale(0, RoundingMode.HALF_DOWN);
			sumVatAdult += convertToInt(adultTax) * adultCount; // diexpress harganya bisa ,5 jadi harus dibuletin ke bawah
		}
