	...
	<div class="row">
	    ...
	    <form method="get" action="${ctx}/admin/users" id="searchForm" class="form-inline">
			<div class="col-xs-9 form-horizontal">
				...
				<select name="roleId" class="form-control" style="width:14em; display: inline;"  >
				 	<option value="" <c:if test="${empty param.roleId})"> 'selected' </c:if> >ALL</option>
	                <c:forEach items="${roles}" var="role">
	                	<option value="${role.id}" ${fn:contains(param.roleId, role.id) ? 'selected' : ''}>${role.name}</option>
                	</c:forEach>
	            </select>
...
<!-- 	double email krn dibedakan utk export report         -->
        ...
        <display:column property="roles" titleKey="user.role" media="all" style="font-size: small"/>
        <display:column property="created_date" titleKey="user.regDate" media="all" format="{0,date,dd-MMM-yyyy}" sortable="true" style="font-size: small"/>
